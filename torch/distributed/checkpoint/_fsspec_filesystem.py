...
class FsspecReader(FileSystemReader):
        ...
        **kwargs,
        ...
        Args:
            ...
            max_batch_bytes: Maximum cumulative byte size requested per batched
                ...
                to be live at peak.
            range_bytes: Target size of one requested range. Contiguous items are
                merged into ranges up to this size and larger items are split,
                since each range has a fixed cost but too few ranges limit
                parallelism. Defaults to 16 MiB.
    ...
    def read_data(self, plan: LoadPlan, planner: LoadPlanner) -> Future[None]:
        ...
        # A group is a run of ranges that must land in the same batch: either
        # one range holding several contiguous items, or one item split into
        # several ranges. Each member maps to (range index, lo, hi) segments.
        groups = []
        span = None
        for req in reqs:
            ...
            if item_md.length > self.range_bytes:
                span = None
                n = -(-item_md.length // self.range_bytes)
                cuts = [start + i * item_md.length // n for i in range(n + 1)]
                ranges = list(itertools.pairwise(cuts))
                segs = [(i, 0, e - s) for i, (s, e) in enumerate(ranges)]
                groups.append((path, ranges, [(req, segs)]))
            elif (
                span is not None
                and span[0] == path
